...
export const RegisterForm = () => {
  ...
  return (
    ...
    <form onSubmit={handleSubmit((formData) => parseRegisterData(formData))}
      ...
      <span className="text-paragraph pb-2 pt-6">Welcome, Administrator!</span>
      {loading ? <span>LOADING</span>
        : <div className="flex flex-col gap-3">
          <div className="relative">
            <input className="bg-grey h-20 p-6 text-paragraph w-full"
              type="text" placeholder="NAME" {...register("name")} />
            {errors.name && <p className="absolute bottom-0 left-5">{errors.name.message}</p>}
          </div>
          <div className="relative">
            <input className="bg-grey h-20 p-6 text-paragraph w-full"
              type="email" placeholder="E-MAIL" {...register("email")} />
            {errors.email && <p className="absolute bottom-0 left-5">{errors.email.message}</p>}
          </div>
          <div className="relative">
            <input className="bg-grey h-20 p-6 text-paragraph w-full"
              type="text" placeholder="PASSWORD" {...register("password")} />
            {errors.password && <p className="pl-6">{errors.password.message}</p>}
          </div>
          <div className="relative">
            <input className="bg-grey h-20 p-6 text-paragraph w-full"
              type="password" placeholder="CONFIRM PASSWORD" {...register("confirmPassword")} />
            {errors.confirmPassword && <p className="absolute bottom-0 left-5">{errors.confirmPassword.message}</p>}
          </div>
          <button type="submit"
